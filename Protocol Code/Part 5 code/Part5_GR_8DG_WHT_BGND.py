# Green vs. Red at 0, 15 Degrees, Binary Decision, 8 Degree Circle, White Background
...
angles = [0,15] # Retinal eccentricity (opening angle) values to test
...
if recordData:
    ...
    fileName = os.path.join(OUTPATH,\
        (expInfo['Name'] + expInfo['Trial Number'] + '_' + 'greenvsred' + '_' + date + '_' + expName + '.csv'))
    
    # Print column headers if the output file does not exist
    if not os.path.isfile(fileName):
        csvOutput(["Color","Direction", "Eccentricity (degrees)", "Reaction Time (s)", "Correctness", "Name", "Trial", "Circle Size (degrees)", "Experiment Name", "Background Color"]) 
...
def instructions():
    ...
    genDisplay({'text': 'Press F if you see a GREEN circle.',\
        'xPos': 0, 'yPos': 5, 'heightCm': 1*heightMult, 'color': 'black'}).draw()
    genDisplay({'text': 'Press J if you see a RED circle.',\
        'xPos': 0, 'yPos': 3, 'heightCm': 1*heightMult, 'color': 'black'}).draw()
    genDisplay({'text': 'Press spacebar to continue.',\
        'xPos': 0, 'yPos': -2, 'heightCm': 1*heightMult, 'color': 'black'}).draw()
    win.flip()
...
def expBreak():
    dispInfo = {'text': 'Break', 'xPos': 0, 'yPos': 4, 'heightCm': 3, 'color': 'black'}
    breakText = genDisplay(dispInfo)
    dispInfo = {'text': '', 'xPos': 0, 'yPos': -1, 'heightCm': 3, 'color': 'black'}
    for i in range(30):
        breakText.draw()
...
for pair in pairs: # Loop through the list of pairs
    ...
    if checkResponse(key[0], trialInfo['color']):
        csvOutput([trialInfo['color'], trialInfo['dir'], trialInfo['angle'], reactionTime, correct, expInfo['Name'], expInfo['Trial Number'], 4, "GR", "WHITE"])
    else:
        csvOutput([trialInfo['color'], trialInfo['dir'], trialInfo['angle'], reactionTime, incorrect, expInfo['Name'], expInfo['Trial Number'], 4, "GR", "WHITE"])
