# Eccentricity Dependence for RED, Binary Decision, 1/2/3 Colors
...
yellow=[255,255,0]
colors = [red]
trials = 4 # Number of trials to run
...
if recordData:
    ...
    expName = 'Color_RT_Eccentricity'
    expInfo = {'Name': '', 'Attention (Covert or Overt)': ''}
    ...
    fileName = os.path.join(OUTPATH,\
        (expInfo['Name'] + '_' + expInfo['Attention (Covert or Overt)'] + '_' + 'red' + '_' + date + '_' + expName + '.csv'))
    
    # Print column headers if the output file does not exist
    if not os.path.isfile(fileName):
        csvOutput(["Color","Direction", "Eccentricity (degrees)", "Reaction Time (s)", "Correctness", "1/2/3 Colors"]) 
...
def instructions():
    ...
    genDisplay({'text': 'Press F if you see a RED circle.',\
        'xPos': 0, 'yPos': 5, 'heightCm': 1*heightMult, 'color': 'white'}).draw()
    genDisplay({'text': 'Press J if you see any other colors.',\
...
for pair in pairs: # Loop through the list of pairs
    ...
    if checkResponse(key[0], trialInfo['color']):
        csvOutput([trialInfo['color'], trialInfo['dir'], trialInfo['angle'], reactionTime, correct, 1])
    else:
        csvOutput([trialInfo['color'], trialInfo['dir'], trialInfo['angle'], reactionTime, incorrect, 1])
...
yellow=[255,255,0]
colors = [red, green]
trials = 12 # Number of trials to run
...
for pair in pairs: # Loop through the list of pairs
    ...
    if checkResponse(key[0], trialInfo['color']):
        csvOutput([trialInfo['color'], trialInfo['dir'], trialInfo['angle'], reactionTime, correct, 2])
    else:
        csvOutput([trialInfo['color'], trialInfo['dir'], trialInfo['angle'], reactionTime, incorrect, 2])
...
trials = 15 # Number of trials to run
...
for pair in pairs: # Loop through the list of pairs
    ...
    if checkResponse(key[0], trialInfo['color']):
        csvOutput([trialInfo['color'], trialInfo['dir'], trialInfo['angle'], reactionTime, correct, 3])
    else:
        csvOutput([trialInfo['color'], trialInfo['dir'], trialInfo['angle'], reactionTime, incorrect, 3])
