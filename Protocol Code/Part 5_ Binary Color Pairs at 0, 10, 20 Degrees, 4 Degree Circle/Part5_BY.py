# Blue vs. Yellow at 0, 10, 20 Degrees, Binary Decision, 4 Degree Circle
...
if recordData:
    ...
    fileName = os.path.join(OUTPATH,\
        (expInfo['Name'] + expInfo['Trial Number'] + '_' + 'bluevsyellow' + '_' + date + '_' + expName + '.csv'))
...
for pair in pairs: # Loop through the list of pairs
    ...
    if checkResponse(key[0], trialInfo['color']):
        csvOutput([trialInfo['color'], trialInfo['dir'], trialInfo['angle'], reactionTime, correct, expInfo['Name'], expInfo['Trial Number'], 4, "BY"])
    else:
        csvOutput([trialInfo['color'], trialInfo['dir'], trialInfo['angle'], reactionTime, incorrect, expInfo['Name'], expInfo['Trial Number'], 4, "BY"])
