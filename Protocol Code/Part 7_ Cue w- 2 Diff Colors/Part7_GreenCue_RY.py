# Red vs. Yellow at 0 Degrees, Binary Decision, 8 Degree Circle, Black Background, Green Circle Cue
...
angles = [0] # Retinal eccentricity (opening angle) values to test
...
yellow=[255,255,0]
colors = [yellow, red]
trials = 25 # Number of trials to run
...
if recordData:
    ...
    fileName = os.path.join(OUTPATH,\
        (expInfo['Name'] + expInfo['Trial Number'] + '_' + 'redvsyellow' + '_' + 'blk_bgnd' + '_' + 'greenCircleCue' + '_' + date + '_' + expName + '.csv'))
...
for pair in pairs: # Loop through the list of pairs
    ...
    if checkResponse(key[0], trialInfo['color']):
        csvOutput([trialInfo['color'], trialInfo['dir'], trialInfo['angle'], reactionTime, correct, expInfo['Name'], expInfo['Trial Number'], 8, "RY", "BLACK", "Green Circle"])
    else:
        csvOutput([trialInfo['color'], trialInfo['dir'], trialInfo['angle'], reactionTime, incorrect, expInfo['Name'], expInfo['Trial Number'], 8, "RY", "BLACK", "Green Circle"])
