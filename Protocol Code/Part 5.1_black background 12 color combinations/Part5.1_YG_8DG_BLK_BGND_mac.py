# Yellow vs. Green at 0, 10, 20 Degrees, Binary Decision, 8 Degree Circle, Black Background
...
if recordData:
    ...
    fileName = os.path.join(OUTPATH,\
        (expInfo['Name'] + expInfo['Trial Number'] + '_' + 'yellowvsgreen' + '_' + 'blk_bgnd' + '_' + date + '_' + expName + '.csv'))
...
for pair in pairs: # Loop through the list of pairs
    ...
    if checkResponse(key[0], trialInfo['color']):
        csvOutput([trialInfo['color'], trialInfo['dir'], trialInfo['angle'], reactionTime, correct, expInfo['Name'], expInfo['Trial Number'], 8, "YG", "BLACK"])
    else:
        csvOutput([trialInfo['color'], trialInfo['dir'], trialInfo['angle'], reactionTime, incorrect, expInfo['Name'], expInfo['Trial Number'], 8, "YG", "BLACK"])
