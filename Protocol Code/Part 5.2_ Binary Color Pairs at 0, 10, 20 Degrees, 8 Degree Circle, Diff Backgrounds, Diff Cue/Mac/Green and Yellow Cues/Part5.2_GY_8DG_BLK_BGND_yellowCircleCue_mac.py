# Green vs. Yellow at 0, 10, 20 Degrees, Binary Decision, 8 Degree Circle, Black Background, Yellow Circle Cue
...
if recordData:
    ...
    fileName = os.path.join(OUTPATH,\
        (expInfo['Name'] + expInfo['Trial Number'] + '_' + 'greenvsyellow' + '_' + 'blk_bgnd' + '_' + 'yellowCircleCue' + '_' + date + '_' + expName + '.csv'))
...
def instructions():
    ...
    genDisplay({'text': 'Press J if you see a YELLOW circle.',\
...
def expBreak():
    ...
    for i in range(30):
        breakText.draw()
        dispInfo['text'] = str(30-i) + ' seconds GY'
...
for pair in pairs: # Loop through the list of pairs
    ...
    if checkResponse(key[0], trialInfo['color']):
        csvOutput([trialInfo['color'], trialInfo['dir'], trialInfo['angle'], reactionTime, correct, expInfo['Name'], expInfo['Trial Number'], 8, "GY", "BLACK", "Yellow Circle"])
    else:
        csvOutput([trialInfo['color'], trialInfo['dir'], trialInfo['angle'], reactionTime, incorrect, expInfo['Name'], expInfo['Trial Number'], 8, "GY", "BLACK", "Yellow Circle"])
