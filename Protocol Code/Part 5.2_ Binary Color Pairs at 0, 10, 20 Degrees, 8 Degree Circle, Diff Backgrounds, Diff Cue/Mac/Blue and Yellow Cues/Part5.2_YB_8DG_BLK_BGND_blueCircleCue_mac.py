# Yellow vs. Blue at 0, 10, 20 Degrees, Binary Decision, 8 Degree Circle, Black Background, Blue Circle Cue
...
if recordData:
    ...
    fileName = os.path.join(OUTPATH,\
        (expInfo['Name'] + expInfo['Trial Number'] + '_' + 'yellowvsblue' + '_' + 'blk_bgnd' + '_' + 'blueCircleCue' + '_' + date + '_' + expName + '.csv'))
...
cross = visual.Circle(
    ...
    size=(1, 1),
    ori=0, pos=(centerX, 0),
    lineWidth=1, lineColor=[0,0,255], lineColorSpace='rgb255',
...
for pair in pairs: # Loop through the list of pairs
    ...
    if checkResponse(key[0], trialInfo['color']):
        csvOutput([trialInfo['color'], trialInfo['dir'], trialInfo['angle'], reactionTime, correct, expInfo['Name'], expInfo['Trial Number'], 8, "YB", "BLACK", "Blue Circle"])
    else:
        csvOutput([trialInfo['color'], trialInfo['dir'], trialInfo['angle'], reactionTime, incorrect, expInfo['Name'], expInfo['Trial Number'], 8, "YB", "BLACK", "Blue Circle"])
