# Eccentricity Dependence for YELLOW, Binary Decision, 2 Colors
...
blue = [0,0,255]
colors = [yellow, blue]
trials = 7 # Number of trials to run
...
if recordData:
    ...
    expInfo = {'Name_Center/Left/Right': ''}
    ...
    fileName = os.path.join(OUTPATH,\
        (expInfo['Name_Center/Left/Right'] + '_' + 'yellow2' + '_' + date + '_' + expName + '.csv'))
...
def instructions():
    ...
    genDisplay({'text': 'Press J for any other colors.',\
...
for pair in pairs: # Loop through the list of pairs
    ...
    if checkResponse(key[0], trialInfo['color']):
        csvOutput([trialInfo['color'], trialInfo['dir'], trialInfo['angle'], reactionTime, correct])
    else:
        csvOutput([trialInfo['color'], trialInfo['dir'], trialInfo['angle'], reactionTime, incorrect])
